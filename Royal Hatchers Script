...
TabShop:Toggle("Auto Buy Merchant (Normal)", false, "AutoMerchant", "Buys all normal merchant slots", function(Value)
...
TabShop:Toggle("Auto Buy Merchant (1M Event)", false, "Auto1MMerchant", "Buys all 1M event merchant slots", function(Value)
    Flags.Auto1MMerchant = Value
    task.spawn(function()
        while Flags.Auto1MMerchant do
            pcall(function()
                for i = 1, 15 do
                    Functions.BuyMerchant:InvokeServer("1MMerchant", i)
                end
            end)
            task.wait(2)
        end
